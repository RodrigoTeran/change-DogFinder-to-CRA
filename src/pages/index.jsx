...
const Index = ({
  ...
  const checkIfBodyContactWithMenuBar = () => {
    // Cada vez que el usuario interactua con la ventana
    // Posicionarlo correctamente
    setTops({
      // Acomodar los tops al cambiar ventana
      topL1: window.innerHeight,
      topL2: window.innerHeight * 2.3,
      topL3: window.innerHeight * 3.6,
    });
    var clientPosition =
      parseInt(document.querySelector(".Top-menu-bar").clientHeight) +
      document.scrollingElement.scrollTop;
    // ---------------------------------------Sección 1---------------------------------------
    var endLayout_body_container =
      getTop(".index-page-body-container") +
      parseInt(
        document.querySelector(".index-page-body-container").clientHeight
      );
    if (clientPosition >= getTop(".index-page-body-container")) {
      // esta en la sección 1
      if (document.scrollingElement.scrollTop <= endLayout_body_container) {
        // Hasta donde termina la sección 1
        // Si esta abierto el responsive menu bar body
        checkIfResponsiveMenuBarIsOpen();
      } else {
        // Ya se pasó la sección 1
        // ---------------------------------------Segunda Sección---------------------------------------
        var endLayout_body_container_2 =
          getTop(".index-page-body-container-2") +
          parseInt(
            document.querySelector(".index-page-body-container-2").clientHeight
          );
        if (clientPosition >= getTop(".index-page-body-container-2")) {
          // esta en la sección 2
          if (
            document.scrollingElement.scrollTop <= endLayout_body_container_2
          ) {
            // Hasta donde termina la sección 2
            // Si esta abierto el responsive menu bar body
            checkIfResponsiveMenuBarIsOpen();
          } else {
            // Ya se pasó la sección 2
            // ---------------------------------------Tercera Sección---------------------------------------
  ...
  const checkIfClientIsInWhatSection = () => {
    ...
    if (
      ...
      document.querySelector(".text-back-h1").innerHTML =
        "ANTES TENÍAS QUE TENER SUERTE PARA QUE LA <span class='span-text-back-h1'>PERSONA CORRECTA</span> VIERA LA PUBLICACIÓN CORRECTA";
    } else if (
      endLayout_body_container <=
      document.scrollingElement.scrollTop + window.innerHeight
    ) {
      updateBackgroundPagesImagePositionClass({
      ...
      document.querySelector(".text-back-h1").innerHTML =
        "CADA AÑO SE PIERDEN MÁS DE <span class='span-text-back-h1'>3.8 MILLONES</span> DE MASCOTAS EN MÉXICO";
    } else {
      updateBackgroundPagesImagePositionClass({
        // Está en la primera sección
        state_1: true,
        state_2: false,
        state_3: false,
      });
      document.querySelector(".text-back-h1").innerHTML =
        "CUANDO UN PERRO SE PIERDE, TIENE UN 80% DE POSIBILIDAD DE <span class='span-text-back-h1'>NUNCA</span> VOLVER A CASA...";
    }
  };
  const handleWindowAnimations = () => {
    for (const component in animaciones) {
      // Checa las animaciones
      if (
        document.scrollingElement.scrollTop +
          parseInt(document.querySelector(component).clientHeight) *
            animaciones[component].det >=
        getDistanceComponentToDoAnimation(component)
      ) {
        // Es porque ya aparece en el lugar correcto
        if (!animaciones[component].status) {
          // Si no ha pasado esto otra vez
          updateIndexAnimations({
            // Actualiza Redux
            state: false,
            animName: component,
          ...
          setAnimaciones({
            // Actualiza el propio estado de Index
            ...animaciones,
            [component]: {
              status: true,
  ...
  const getDistanceComponentToDoAnimation = (component) => {
    ...
    return (
      getTop(component) -
      window.innerHeight +
      parseInt(document.querySelector(component).clientHeight) / 2
    );
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={`${APP_NAME} es una aplicación que ayuda a rescatar tu perro en caso de extravío.`}
        />
      </Helmet>
      <BackgroundImage /> {/* El Background de Index */}
      {/* Primera Seccion */}
      <div
        className="index-page-body-container"
        style={{ backgroundColor: "rgba(25, 25, 25, 1)", top: `${topL1}px` }}
      >
        <header>
          <IndexHeader />
        </header>
        <section style={{ paddingBottom: "30px" }} className="section-1-index">
          <HeaderSection1Index />
        </section>
      </div>
      {/* Segunda Seccion */}
      ...
      ></ImageInBigView>
      <div
        className="index-page-body-container-2"
        style={{ backgroundColor: "rgba(25, 25, 25, 1)", top: `${topL2}px` }}
      >
        <header>
          <HeaderSection2Index setImgSrc={setImgSrc} setYes={setYes} />
        </header>
        <section>
          <Section2IndexSection1 />
        </section>
      </div>
      {/* Tercera Seccion */}
      <div
        className="index-page-body-container-3"
        style={{ backgroundColor: "rgba(25, 25, 25, 1)", top: `${topL3}px` }}
      >
        <header>
          <HeaderSection3Index />
        </header>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateBackgroundPagesImagePositionClass: (data) => {
      dispatch(updateBackgroundPagesImagePositionClassAction(data));
    },
    updateIndexAnimations: (data) => {
      dispatch(updateIndexAnimationsAction(data));
    },
  };
};
