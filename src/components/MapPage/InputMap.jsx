...
import { TEXT_WANT_SEE_OWN_PREMIUM_PROFILES } from "../../utils/textForBannerOkCancelAction";

import { updateMapArraysAction } from "../../store/reducers/company/actions";

import { updateBannerRedirectWithLinkAction } from "../../store/reducers/layout/actions";
...
const InputMap = ({
  ...
  const [whichViewIsActive, setWhichViewIsActive] = useState(
    "globalPremiumProfiles"
  );
    ...
    if (typeOfObject === "Company") {
      if (companyDataLeftPage["idProfile"]) {
        if (companyDataLeftPage["idProfile"] === idOfObject) {
          ...
          fetchInformationForLeftColumnMapPage(typeOfObject, idOfObject);
        }
      } else {
        fetchInformationForLeftColumnMapPage(typeOfObject, idOfObject);
      }
  ...
  return (
    <div className="map-page-right">
      ...
      <div
        className={`map-page-right-buttons-container ${
          isButtonsContainerOpen ? "open" : "close"
        }`}
      >
        <button
          className="map-page-right-buttons-container-buttons"
          onClick={() => {
            // Ubicacion actual
        ...
        <button
          className="map-page-right-buttons-container-giant-button"
          onClick={() => {
            setButtonsContainerOpen(!isButtonsContainerOpen);
          }}
        >
          <svg
            className={`${isButtonsContainerOpen ? "open" : "close"}`}
          ...
          <>
            ...
            {ownPremiumProfiles.arrayOfInformation.map((profile) => {
              return (
                ...
                >
                  <div
                    className="custom-marker-map"
                    onClick={() => {
                      checkIfNeedToFecthInfo(profile.typeOfData, profile.id);
                    }}
                  >
                    <div
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateMapArrays: (data) => {
      dispatch(updateMapArraysAction(data));
    },
    updateLogin: (data) => {
    ...
    updateBannerRedirectWithLink: (data) => {
      dispatch(updateBannerRedirectWithLinkAction(data));
    },

    updateActualCompanyDataLeftPage: (data) => {
      dispatch(updateActualCompanyDataLeftPageAction(data));
    },
    updateActualProfileDogFoundedDataLeftPage: (data) => {
      dispatch(updateActualProfileDogFoundedDataLeftPageAction(data));
    },
    updateActualProfilePremiumDataLeftPage: (data) => {
      dispatch(updateActualProfilePremiumDataLeftPageAction(data));
    },
