...
import { editPetProfileDogFoundedWhenIsLost } from "../../routes/indexDogFounded";
    ...
    <>
      {petProfile.isPetProfile ? (
        ...
        <>
          ...
          <div
            className={`calendar-container div-calendar-pet-founded-2 ${
              firstAnimCalendar ? "yes" : ""
            } `}
            style={{
              height: isCalendarOpen
                ? `${
                    heightCalendar -
                    parseInt(window.innerWidth <= 1120 ? 50 : 25)
                  }px`
                : "0px",
              marginTop: isCalendarOpen
                ? `${window.innerWidth <= 1120 ? "20px" : "0px"}`
                : "0px",
              marginBottom: isCalendarOpen ? "20px" : "0px",
            }}
          >
            <div
              className={`calendar-animation ${isCalendarOpen ? "open" : ""}`}
            >
              <Calendar
                className={`calendar`}
                onChange={(date) => {
