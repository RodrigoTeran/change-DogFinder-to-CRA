...
const InfoPetProfile = ({
      ...
      fetch(`${petProfile.isPetProfile ? (`${editPetProfileRace}/${petProfile.name}`) : (`${editPetProfileDogFoundedBreed}/${petProfile.name}`)}`, {
        ...
        if (data.status) { // todo bien
          updateSuccessMessagesComponent({
            ...
            description: petProfile.isPetProfile ? ("Se cambió la raza de su mascota con éxito") : ("Se cambió la raza del perrito con éxito")
          });
          updatePetProfile({
  ...
  const [isInputActivatedDogGender, setIsInputActivatedDogGender] = useState(false);
  const changeDogGender = (dogGender) => {
    ...
    if (dogGender === "Buscar") {
      updateFailureMessagesComponent({
        state: true,
        title: "Error",
        description: "Primero debe de escoger un género para poder guardar cambios"
      });
      setIsLoading(false);
    } else {
      fetch(`${petProfile.isPetProfile ? (`${editPetProfileGender}/${petProfile.name}`) : (`${editPetProfileDogFoundedGender}/${petProfile.name}`)}`, {
        ...
        if (data.status) { // todo bien
          updateSuccessMessagesComponent({
            ...
            description: petProfile.isPetProfile ? ("Se cambió el género de su mascota con éxito") : ("Se cambió el género del perrito con éxito")
          });
          updatePetProfile({
  ...
  return (
    <>
      <InputDataList
        title={`${petProfile.isPetProfile ? ("Escoje la raza de tu mascota") : ("Escoje la raza del perro extraviado")}`}
      ...
      <InputDataList
        title={`${petProfile.isPetProfile ? ("Escoje el género de tu mascota") : ("Escoje el género del perro extraviado")}`}
        subtext="Pulsa el botón de Buscar"
        closeFunction={() => {
      ...
      {window.innerWidth >= 1121 ? (<></>) : (
        <div className={`pet-profile-page-delete-info text-center`}>
          {`${petProfile.isPetProfile ? (`Información de tu mascota`) : (`Información del perro extraviado`)}`}
        </div>
      )}
      <div className={`info-pet-profile row`}>
        ...
        <CardInfoPetProfile
          title={`${petProfile.isPetProfile ? ("Residencia") : ("¿Dónde lo hallaste?")}`}
