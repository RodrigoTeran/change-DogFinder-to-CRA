        ...
        fetch(`${petProfile.isPetProfile ? (`${editPetName}/${petProfile.name}`) : (`${editPetsDogFoundedName}/${petProfile.name}`)}`, {
          ...
          if (data.status) { // todo bien
            ...
            updateSuccessMessagesComponent({
              ...
              description: petProfile.isPetProfile ? (`Se cambió el nombre de tu mascota con éxito`) : (`Se cambió el apodo del perro con éxito`),
            })
          } else {
            updateFailureMessagesComponent({
              state: true,
              title: "Error al cambiar el nombre",
              description: petProfile.isPetProfile ? (`Ese nombre ya lo tiene registrado en otra de sus mascotas`) :
  ...
  return (
    <>
      ...
      <div className="pet-profile-page-header-h1">
        {`${petProfile.isPetProfile ? (`Nombre de tu mascota: `) : (`Apodo del perrito: `)}`}
        <span>{petProfile.name}</span>
        <div
          title={`${petProfile.isPetProfile ? ("Cambiar nombre de mascota") : (`Cambiar apodo del perrito`)}`}
          onClick={openInput}>
