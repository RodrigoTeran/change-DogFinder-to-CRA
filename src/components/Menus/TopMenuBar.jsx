...
const Menubar = ({
  ...
  const interactResponsiveMenuBar = () => {
    // Cerrar o abrir el Responsive Menu Bar
  ...
  return (
    <>
      ...
      <nav
        className={`navbar navbar-expand-lg fixed-top Top-menu-bar ${
          topMenuBar.activated ? "activated" : ""
        }`}
        style={{ visibility: firstResponseUserAPI ? "visible" : "hidden" }}
      >
        <div className="container-fluid">
          <ul className="navbar-nav">
            <li>
              <Link to="/" className="link-after-effect" title={`${APP_NAME}`}>
                {APP_NAME_CAPITALS}
              </Link>
            </li>
          </ul>
          <div className="menu-bar-collapse content-top-menu-bar">
            <ul className="navbar-nav pr-5">
              ...
              <li className="mr-3">
                {auth ? (
                  <Link
                    to="/registro/mascota/encontrada"
                    className="link-after-effect"
                    title="Encontré a un perro"
                    style={{ cursor: "pointer" }}
                  >
                    ENCONTRÉ A UN PERRO
                  </Link>
                ...
                <>
                  ...
                  >
                    <Link
                      ...
                      style={{ cursor: "pointer" }}
                    >
                      <div className="Top-menu-bar-image-profile notificaciones-container">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 448 512"
                        >
          ...
          <div
            className="hamburger"
            title="Menu"
            onClick={interactResponsiveMenuBar}
          >
            {/* Si esta abierto el responsive menu bar content, se muestra una X, si no, la hamburger original */}
            {line1 ? (
              <div className="line line1 x-line-1"></div>
            ) : (
              <div className="line line1"></div>
            )}
            {line2 ? (
              <div className="line line2 x-line-2"></div>
            ) : (
              <div className="line line2"></div>
            )}
            {line3 ? (
              <div className="line line3 x-line-3"></div>
            ) : (
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateResponsiveMenuBarBodyTrans: (data) => {
      dispatch(updateResponsiveMenuBarBodyTransAction(data));
    },
    updateLogin: (data) => {
