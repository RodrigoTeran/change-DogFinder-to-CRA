...
const CardsForCredentialsCompany = ({
  title,
  text,
  children
}) => {
  return (
    <div className="card-credentials-company">
      <div className="card-credentials-company-cols-2">
        <div className="card-credentials-company-col-2">
          {children}
        </div>
      ...
      <div className="card-credentials-company-cols">
        {text}
      </div>
    </div>
  )
...
const CredentialsCompany = ({
  ...
  return (
    <>
      ...
      <div className={`image-pet-profile-instructions image-pet-profile-instructions-2 ${yesInstructions ? ("open") : ("close")}`} style={{
        marginTop: "10px",
        width: window.innerWidth < 1050 ? ("300px") : ("95%"),
        marginLeft: `${window.innerWidth < 1050 ? ("calc(50% - 150px)") : ("0px")}`,
      ...
      >
        <div className="image-pet-profile-instructions-icon">
          <div onClick={() => { setInstructions(!yesInstructions) }} title="Instrucciones" style={{
            display: "flex",
            alignItems: "center",
            justifyContent: `${window.innerWidth < 1050 ? (yesInstructions ? ("left") : ("center")) : ("left")
              }`
          }}>
        ...
        <div className={`${yesInstructions ? ("open") : ("close")} image-pet-profile-instructions-text`}>
          A tus clientes como empresa les puedes dar el código de descuento. Ellos tienen un 10% de descuento en la compra de un perfil premium.
          Y al final de cada mes se cuentan cuantos usuarios nuevos usaron su código. Se toma ese número de nuevos usuarios con código de ustedes
          y se multiplica por 50, y esa cantidad de dinero
          se les da a ustedes como empresa por el trabajo de difundir la plataforma. Es decir... los usuarios se ahorran el 10%, ustedes como empresa
          ganan ese 10% por cada usuario que use el código al final de cada mes. Ejemplo: Si ustedes al final de un mes registraron a 10 nuevos usuarios
          que usaron el código para comprar el premium, se les da a ustedes $500.
        </div>
      </div>
    </>
  )
