...
import { postUserInCompany } from "../../routes/company";
import { updateUserAction } from "../../store/reducers/user/actions";
...
import { getBannerProfileContactInfo } from "../../store/reducers/layout/selector";
...
const HeaderProfilePage = ({
        ...
        .then((data) => {
          ...
          if (data.status === "true") {
            ...
            updateFailureMessagesComponent({
              ...
              title: "Error",
              description:
                "La clave ya expiró o no coincide. Vuelva a intentarlo o envie de nuevo el correo",
            });
          } else {
            updateFailureMessagesComponent({
              state: true,
      ...
      .then((data) => {
        ...
        if (data.status === "true") {
          ...
          updateFailureMessagesComponent({
            ...
            title: "Error",
            description:
              "El correo no es válido, puede ser que lo hayas escrito mal",
          });
        }
      });
  };
  ...
  const [
    valueInputForCompanyRegister,
    setValueInputForCompanyRegister,
  ] = useState("");
    ...
    if (hack) {
      ...
      fetch(postUserInCompany, {
        ...
        .then((data) => {
          ...
          } else if (data.status === "yaEstaConOtra") {
            // ya esta en otra
            updateFailureMessagesComponent({
              state: true,
              title: "Error",
  ...
  useEffect(() => {
    if (
      ...
      bannerProfileContactInfo.okButton === true
    ) {
      const hack = checkFuckingHack(
        bannerProfileContactInfo.inputInfoFromBanner,
        ["@", "."]
      );
      let status = false;
      if (hack) {
        ...
        for (
          var i = 0;
          i < bannerProfileContactInfo.inputInfoFromBanner.length;
          i++
        ) {
    ...
    if (
      bannerProfileContactInfo.isDisplayed.fromWho === "Whatsapp" &&
      bannerProfileContactInfo.okButton === true
    ) {
      const hack = checkFuckingHack(
        bannerProfileContactInfo.inputInfoFromBanner,
  ...
  return (
    <>
      <div className="d-flex row justify-content-around align-items-center row-header-profile-page">
        ...
        <div
          style={{ fontSize: "1rem" }}
          className="col-lg-6 col-md-6 col-sm-10 my-3 row-header-profile-page-col"
        >
          <div className="d-flex justify-content-center">
            ...
            >
              {username}
              {email ? (
                <span style={{ fontSize: ".85rem", wordBreak: "break-word" }}>
                  {" "}
                  Correo:{" "}
      ...
      {!userCompany.name ? (
        <>
          <InputTextComponent
            closeInput={() => {
              setIsViewOnCompany(false);
            }}
            closeStyle={!isViewOnCompany}
            maxLenghtInput={15}
            onChangeFunction={onChangeInputForCompanyRegister}
            clickFunction={() => {
              if (!isLoading && !isLoading2) {
          ...
          >
            <div className="input-layout-text-title">
              Escribe el código de registro
            </div>
            <div className="input-layout-text-subtitle">
              Máximo número de caracteres: 15
            </div>
          </InputTextComponent>
          {isLoading2 ? (
            <div
          ...
          <div className="profile-page-contact-profile-contacts-container-row">
            <div className="profile-page-contact-profile-contacts-container-col">
              <ButtonWhiteRectangle
                ...
                height="50px"
                fontSize="1rem"
                clickFunctionAnotherOne={ProfileContactMail}
                mt="mt-0"
            ...
            <div className="profile-page-contact-profile-contacts-container-col profile-page-contact-profile-contacts-container-col-2">
              <ButtonWhiteRectangle
                ...
                height="50px"
                fontSize="1rem"
                clickFunctionAnotherOne={ProfileContactWhatsapp}
                mt="mt-0"
      ...
      <div
        className={`banner-email-contact-key-input ${
          isViewOnCompany && userCompany.name
            ? userCompany.correoCompaniaEspacioParaKey
              ? "open"
              : "close"
            : emailForContactActiveKey
            ? "open"
      ...
      >
        ...
        <div className="banner-email-contact-key-input-info">
          Introduce la clave que te llego al correo. Si no te ha llegado el
          correo, lo más probable es que lo hayas escrito mal. Si es así, solo
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateBannerProfileContactInfo: (data) => {
      dispatch(updateBannerProfileContactInfoAction(data));
    },
